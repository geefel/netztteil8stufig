...
#define RELAY_3    PORTBbits.RB5
#define RELAY_4    PORTBbits.RB6
#define RELAY_5    PORTBbits.RB7
#define TASTE_1    PORTAbits.RA0
#define TASTE_2    PORTAbits.RA1
#define TASTE_3    PORTAbits.RA2
#define TASTE_4    PORTAbits.RA3
#define TASTE_5    PORTAbits.RA4
...
int main(void) {
    ...
    TRISBbits.TRISB5 = SET_OUTPUT;  //Relay_3
    TRISBbits.TRISB6 = SET_OUTPUT;  //Relay_4
    TRISBbits.TRISB7 = SET_OUTPUT;  //Relay_5
    TRISAbits.TRISA0 = SET_INPUT;   //Taste_1
    TRISAbits.TRISA1 = SET_INPUT;   //Taste_2
    TRISAbits.TRISA2 = SET_INPUT;   //Taste_3
    TRISAbits.TRISA3 = SET_INPUT;   //Taste_4
    TRISAbits.TRISA4 = SET_INPUT;   //Taste_5
    ...
    while(1) {
        ...
        if (TASTE_3 == IS_LOW) {
            ...
            RELAY_3 = SET_HIGH;
            sendDat(3);
            while(TASTE_3 == IS_LOW) ;
        }
        if (TASTE_4 == IS_LOW) {
            clearAllRelay();
            RELAY_4 = SET_HIGH;
            sendDat(4);
            while(TASTE_4 == IS_LOW) ;
        }
        if (TASTE_5 == IS_LOW) {
            clearAllRelay();
            RELAY_5 = SET_HIGH;
            sendDat(5);
            while(TASTE_5 == IS_LOW) ;
        }
        if (isrInfo) {
            sendDat(0);  //0: all off
            isrInfo = 0;
